...
function showServerPicker() {
  sessionStorage.removeItem('current_guild'); currentGuild = null;
...
function showDashboard() {
  ...
  populateUserUI(); applyAdminVisibility(); loadSettings(); loadChannels(); fetchBoostStats(); fetchHealth(); fetchHealth();
}
...
async function bkPreview(backupId){
  const det=document.getElementById('bk-detail');if(!det)return;det.style.display='block';det.scrollIntoView({behavior:'smooth',block:'start'});
...
async function mcLoadSettings() {
  ...
  // config.yml preview elements
  const cfgUrl = document.getElementById('mc-cfg-url');
  if (cfgUrl) cfgUrl.textContent = '"' + webhookUrl + '"';
  const cfgGid = document.getElementById('mc-cfg-gid');
  if (cfgGid) cfgGid.textContent = gid;
    ...
    ['mc-chat-channel', 'mc-log-channel'].forEach(selId => {
      ...
      if (_cachedChannels && _cachedChannels.length) {
        ...
        _cachedChannels.forEach(ch => {
          const c = ch.category || 'Uncategorized';
          if (!cats[c]) cats[c] = [];
          cats[c].push(ch);
        });
...
// ── Download pre-filled config.yml for NinjuPlugin ───────────
function mcDownloadConfig() {
  const gid = (currentGuild && currentGuild.id) ? currentGuild.id : '';
  if (!gid) return showToast('Pick a server first', 'error');
  const url = BOT_API + '/minecraft/webhook';
  const yaml =
`# ── NinjuPlugin · config.yml ─────────────────────────────────
# Generated by NinjuBot Dashboard
# Upload this to: plugins/NinjuPlugin/config.yml on your Aternos server
...
`;
  ...
  showToast('⬇ config.yml downloaded!');
}
